...
默认从 CDN 加载 **UMD** 构建。若在 **Vite** 等更适合 ESM 的打包环境中使用，请在 `options` 中设置 `useESM: true`。

### WASM 专有选项

在 `HlsDownloader` 构造函数的 `options` 对象中传入：
...
可在 `HlsDownloader` 的 `options` / `setOptions` 中传入（会合并进每次 `download()`）：

| 选项 | 类型 | 描述 |
|------|------|------|
| `aria2` | `object` | 可选。设置 `enabled: true` 以使用 **aria2**（CLI + 会话文件）拉取分片；可配置 `path`、`maxConcurrentDownloads` 等，详见**适配器 API**。 |

分片并发由每次 **`downloadConcurrency`**（未传则用适配器 **`chunkDownloadConcurrency`**）控制；失败重试由 **`maxRetry`** 对应 aria2 的 `--max-tries`。

`headers` 会按分片写入 aria2 输入文件（`header=Name: value`）。若未安装 aria2 或 aria2 失败，`download()` 会抛出明确错误。
