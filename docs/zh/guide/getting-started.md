...
**方式一：聚合包 `@logosww/hls-downloader`（推荐）**
...
`@logosww/hls-downloader` 依赖 `@hls-downloader/core`、`@hls-downloader/shared`、`@hls-downloader/adapters`；`@hls-downloader/core` 已依赖 `@hls-downloader/shared`。使用适配器时均需能解析到 `@hls-downloader/adapters` 中的 wasm/rust 实现。
...
| 包 / 入口 | 用途 |
|-----------|------|
| `@logosww/hls-downloader` | 聚合入口：默认导出 `HlsDownloader`，并再导出 `shared`、WASM/Rust 适配器等 |
| `@logosww/hls-downloader/core` | 同 `@hls-downloader/core` |
| `@logosww/hls-downloader/shared` | 同 `@hls-downloader/shared` |
| `@logosww/hls-downloader/adapters`、`.../wasm`、`.../rust` | 同 `@hls-downloader/adapters` 及子路径 |
