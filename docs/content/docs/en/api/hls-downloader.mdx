---
title: 'HlsDownloader'
description: 'The HlsDownloader facade class — constructor, options, and methods.'
---

The main facade class for downloading HLS streams. Imported from `@hls-downloader/core`.

<Callout type="info" title="Default download path (transmux)">
...
Re-encoding is opt-in via `transcode` — see [Adapter API](/api/adapters). BrowserAdapter loads its hls-transmux WASM module in `init()`; NodeAdapter is a no-op (FFmpeg loads on demand). `parseHls()` never starts those engines.
...
| _(adapter-specific)_ | —                                    | See [Adapter API](/api/adapters)                             |
...
<Callout type="info" title="BrowserAdapter & NodeAdapter">
  `downloadToStream()` is supported on both **`BrowserAdapter`** and **`NodeAdapter`**, but the two
  adapters have **different streaming semantics** (see below). On `BrowserAdapter` the fMP4 bytes
  flow via `onChunk` as fragmented MP4; on `NodeAdapter` bytes flow with real backpressure.
</Callout>
...
| `maxRetry`            | `number`                 | Total attempts including the first request; effective on Browser and Node streaming paths                                                                                                      |
| `operationId`         | `string`                 | Optional caller-defined event correlation ID; generated automatically when omitted                                                                                                             |
| `downloadConcurrency` | `number`                 | Concurrent segment downloads                                                                                                                                                                   |
...
Returns `{ totalSegments: number, operationId: string }`.
...
const server = createServer(async (req, res) => {
  ...
  await writeFile('output.mp4', Buffer.concat(fileChunks.map((c) => Buffer.from(c))));
});

server.listen(3000);
```

<Callout type="info" title="Recommended approach for stream + persist in parallel">
...
<Callout type="info" title="Resume not supported">
  The streaming path writes to a non-seekable sink, so `resume` is not supported. If the stream
  fails midway, restart from the beginning.
</Callout>
...
**Does not load FFmpeg on `BrowserAdapter`.** On `NodeAdapter`, FFmpeg may load only if lightweight extraction fails — see [Adapter API](/api/adapters).
...
See [VariantSelectOptions](/api/types#variantselectoptions) for the full option shape. `selectBestVariant` and the `VariantSelector` type are exported from `@hls-downloader/shared` for custom strategies.
...
Browser-only, incremental fMP4 output with backpressure. Source codecs are preserved; per-call `transcode` is rejected and global transcode settings are ignored. Check `downloader.capabilities.writableOutput === true`; Node and adapters without this optional capability reject with `UNSUPPORTED_OUTPUT` before acquiring the writer.

The SDK owns the acquired writer: it awaits each write and closes it on success. `READY_FOR_DOWNLOAD` and promise resolution occur only after close succeeds. Failure/cancellation aborts the writer and releases its lock. Writer acquisition, write and close errors use `OUTPUT_WRITE_FAILED`; cancellation uses `ABORTED` with `name === 'AbortError'`. Each failed operation emits one `ERROR`. A custom sink may retain partial output; arbitrary external writes cannot be rolled back. An uncooperative pending write does not prevent the SDK promise from rejecting on cancellation; its eventual abort is observed asynchronously.

The prefetch window holds at most `downloadConcurrency` in-flight/ready segments, plus the segment being processed and initialization data. Media buffers do not grow with total video length. Playlist metadata still grows with segment count, a large individual segment may require substantial memory, and WASM memory retains its allocation high-water mark. This is not a fixed RSS limit. The fMP4 output omits the trailing `mfra` index.

`download()` still produces a complete Blob URL; callback-based `downloadToStream()` still preloads Browser media and does not await callback promises. Neither has the new bounded-media-buffer guarantee. `persistentOutput` describes the default `download()` result and remains false for Browser; `writableOutput` describes this separate caller-owned destination.

#### Save directly to a file

The caller opens the picker during user activation and passes its writable stream. File System Access availability and permissions are application concerns; the SDK never opens a picker or silently falls back to buffering a Blob. The example assumes `#save` and `#cancel` buttons and a browser with `showSaveFilePicker`; a cancelled controller is one-shot, so create a new one for another operation.
...
const controller = new AbortController();
// Invoke directly from a user click; do not await SDK initialization first.
document.querySelector('#save').addEventListener('click', async () => {
  try {
    ...
    await downloader.downloadToWritable(
      { url: 'https://example.com/video.m3u8', signal: controller.signal },
      writable,
    );
  } catch (error) {
    console.error('Download or saving failed', error);
  }
});
