---
title: 'Adapters'
description: 'Choose and configure BrowserAdapter and NodeAdapter for your runtime.'
---
...
<Callout type="warn" title="Transmux by default">
...
Read the same matrix from `downloader.capabilities` at runtime.

| Capability         | Browser         | Node            |
| ------------------ | --------------- | --------------- |
| Download / stream  | yes / yes       | yes / yes       |
| Configurable retry | yes             | yes             |
| Transcode presets  | h264, hevc, vp9 | h264, hevc, vp9 |
| Byte range         | yes             | yes             |
| AES-128            | no              | no              |
| Persistent output  | no              | yes             |
...
See [Adapter API](/api/adapters) for all fields. Set `enabled: true` to download segments with aria2.
...
<Callout type="warn">
  Do not bundle the Node adapter (native `.node` addon) into browser code. For browser bundles, only
  import the browser subpath.
</Callout>
