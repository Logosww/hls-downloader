...
- Browser `BYTERANGE` and `EXT-X-MAP` resources now use validated range requests, with local slicing as a compatibility fallback for full `200` responses.
- Browser and Node now fail with existing structured errors before downloading media for unsupported encryption, live/event playlists, discontinuities, and alternate renditions.
