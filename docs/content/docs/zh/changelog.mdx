...
- **通用**：下载选项、结果和生命周期事件新增 `operationId`；调用方未指定时自动生成 UUID。
- **通用**：新增 `HlsDownloaderError`、稳定错误码，以及覆盖清单、分片、加密、封装、转码和取消失败的脱敏诊断元数据。
- **通用**：新增只读 `downloader.capabilities`，应用可在运行时检查流式下载、重试、字节范围、AES-128、转码和持久输出能力。
- 新增可编程 HTTP HLS fixture，以及 Browser/Node 共享契约测试，覆盖重定向、headers、重试、响应失败、并发和取消。
