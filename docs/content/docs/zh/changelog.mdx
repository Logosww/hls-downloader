...
- 新增 Browser `downloadToWritable(options, writable)`，支持有界媒体缓冲、异步背压、取消及调用方提供的文件写入流，增量输出 fMP4；目标流关闭完成后才报告成功。
- 新增 `HlsDownloaderWritableOptions`、可选 `capabilities.writableOutput` 和结构化 `UNSUPPORTED_OUTPUT` / `OUTPUT_WRITE_FAILED` 错误。Node 暂不支持写入流输出；该路径拒绝单次转码请求并忽略全局转码设置。

## [3.4.0] - 2026-09-13

### 新增

- 新增 `Playlist.hasAlternateRenditions`，供消费方识别声明了备用轨道的 master 清单。

### 变更

- Browser 的 `BYTERANGE` 与 `EXT-X-MAP` 资源改用经过校验的真实范围请求；服务端返回完整 `200` 响应时仍通过本地切片保持兼容。
- Browser 和 Node 遇到不支持的加密、live/event、discontinuity 或 alternate rendition 时，会在下载媒体资源前返回既有的结构化错误。

### 修复

- Browser 对同一 URL 的不同字节范围分别进行缓存和重试，避免错误复用其他范围的数据。
- 确保 `NodeAdapter.downloadToStream()` 在 Promise 完成前交付全部分块，包括很短的 fMP4 流。
