---
title: '适配器'
description: '为你的运行环境选择并配置 BrowserAdapter 与 NodeAdapter。'
---
...
<Callout type="warn" title="默认 Transmux">
...
可通过 `downloader.capabilities` 在运行时读取同一矩阵。

| 能力           | Browser         | Node            |
| -------------- | --------------- | --------------- |
| 下载 / stream  | 是 / 是         | 是 / 是         |
| 可配置重试     | 是              | 是              |
| 转码预设       | h264、hevc、vp9 | h264、hevc、vp9 |
| Byte range     | 是              | 是              |
| AES-128        | 否              | 否              |
| 持久输出       | 否              | 是              |
| Live recording | 否              | 否              |
...
完整字段见[适配器 API](/zh/api/adapters)。设置 `enabled: true` 以使用 aria2 下载分片。
