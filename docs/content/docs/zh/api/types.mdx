---
title: '类型定义'
description: '@hls-downloader/shared 导出的 TypeScript 类型。'
---
...
`hasAlternateRenditions` 表示 master 清单声明了当前 transmux 路径无法安全合并的备用轨道。
...
从 `@hls-downloader/core` 导出。`AdapterSpecificOptions<T>` 由适配器类型推断（见[适配器 API](/zh/api/adapters)）。
...
`downloadToWritable()` 专用选项；返回含 `operationId` 的 `HlsDownloaderStreamResult`。`AdapterCapabilities.writableOutput?: boolean` 缺省视为不支持。
