---
title: 'HlsDownloader'
description: 'HlsDownloader 门面类 — 构造函数、选项与方法。'
---

用于下载 HLS 流的主门面类。从 `@hls-downloader/core` 导入。

<Callout type="info" title="默认下载路径（Transmux）">
...
重编码通过 `transcode` 显式启用 — 见[适配器 API](/zh/api/adapters)。BrowserAdapter 使用 Mediabunny 与 WebCodecs；NodeAdapter 按需加载原生 FFmpeg。`init()` 与 `parseHls()` 均不会启动这些引擎。
...
| _（适配器专有）_ | —                                    | 见[适配器 API](/zh/api/adapters)          |
...
<Callout type="info" title="BrowserAdapter 与 NodeAdapter">
  `downloadToStream()` 在 **`BrowserAdapter`** 与 **`NodeAdapter`** 上均已实现。在 `BrowserAdapter`
  上，fMP4 字节通过 `onChunk` 推送，适合实时 MSE 播放（例如 `SourceBuffer.appendBuffer`）。
</Callout>
...
| `maxRetry`            | `number`                 | 包含首次请求的总尝试次数；Browser 与 Node 流式路径均生效           |
| `operationId`         | `string`                 | 可选的事件关联 ID；省略时自动生成                                  |
| `downloadConcurrency` | `number`                 | 分片并发下载数                                                     |
| `signal`              | `AbortSignal`            | 协作式取消。当信号中止时，流式 Promise 会被拒绝并抛出 `AbortError` |

返回 `{ totalSegments: number, operationId: string }`。
...
<Callout type="info" title="边推流 + 边落盘的推荐做法">
...
<Callout type="info" title="不支持 resume">
...
**`BrowserAdapter` 不会为此加载 FFmpeg。** `NodeAdapter` 仅在轻量提取失败时可能加载 — 见[适配器 API](/zh/api/adapters)。
...
仅 Browser 支持，按需下载并通过背压输出 fMP4，保留源编码。拒绝单次调用的 `transcode`，忽略全局转码设置。请检查 `downloader.capabilities.writableOutput === true`；Node 或缺少此可选能力的 adapter 在获取 writer 前报 `UNSUPPORTED_OUTPUT`。

SDK 获取并管理 writer，等待每次写入，成功时关闭流；关闭完成后才发出 `READY_FOR_DOWNLOAD` 并 resolve。失败或取消会 abort writer 并释放锁。获取、写入、关闭失败使用 `OUTPUT_WRITE_FAILED`；取消使用 `ABORTED`，且 `name === 'AbortError'`。失败 operation 仅发一次 `ERROR`。自定义目标可能保留部分输出，SDK 无法回滚任意外部写入；未结束的自定义 write 不会阻止 SDK 在取消时 reject，其后续 abort 结果会被异步处理。

预取窗口最多保留 `downloadConcurrency` 个在途或待消费分片，另有当前处理分片及初始化数据。媒体缓冲不随视频总长度增长；playlist 元数据仍随分片数量增长，超大单分片仍需较多内存，WASM 内存会保留分配高水位。这不是固定 RSS 上限。fMP4 输出不包含尾部 `mfra` 索引。

现有 `download()` 仍返回全量 Blob URL；Browser 回调式 `downloadToStream()` 仍预加载媒体且不等待回调 Promise，均不具备新接口的媒体缓冲保证。`persistentOutput` 描述默认 `download()` 的结果，Browser 仍为 false；`writableOutput` 单独描述写入调用方目标的能力。

#### 直接保存文件

调用方在用户点击时打开选择器并传入写入流。应用自行检查 File System Access 可用性与权限；SDK 不弹窗，也不自动回退为全量 Blob。以下示例假定页面有 `#save`、`#cancel` 按钮且浏览器提供 `showSaveFilePicker`；AbortController 取消后不可复用，新 operation 应创建新的 controller。
